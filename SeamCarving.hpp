...
void SeamCarving::CalculatePath() {
    int index = 0; float mini = energy[0][0];
    for(int i=0; i<width; i++) {
        if(mini < energy[0][i]) {
            mini = energy[0][i];
            index = i;
    ...
    for(int i=1;i<height;i++) {
        if(index == 0) {
            ...
            if(energy[i][index-1] <= energy[i][index] && energy[i][index-1] <= energy[i][index+1])    index = index-1;
            else if(energy[i][index] <= energy[i][index-1] && energy[i][index] <= energy[i][index+1])    index = index;
            else    index = index+1;
            path[i] = index;
        }
    }
