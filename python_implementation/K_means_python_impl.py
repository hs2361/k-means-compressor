...
# * selecting random clusters
    def image_preprocessing(self,filepath):
...
# Generate random centroids
    def random_clusters(self,X,n_clusters):
...
# updating centroids
    def update_centroid(self,X,k,clusters):
        n = np.size(X,1)
    ...
    def KMeans(self,X,initial_centroids,max_iter):
        ...
        for i in range(1,max_iter):
            clusters = self.cluster_assignment(X,centroids)
            centroids = self.update_centroid(X,k,clusters)
        return clusters,centroids

    def main(self,filepath,n_clusters,max_iter):
        Img,rows,cols = self.image_preprocessing(filepath)
        init_cent = self.random_clusters(Img,n_clusters)
        _clusters,_centroids = self.KMeans(Img,init_cent,max_iter)
        _clusters = self.cluster_assignment(Img,_centroids)
